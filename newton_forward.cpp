/* THIS PROGRAM IMPLEMENTS THE NEWTON'S FORWARD INTERPOLATION */
...
int main()
{
    ...
    double x0=x[1],y0=y[1];
    int k=1;
    for(int i=2;i<=n;i++)
        if(x[i]<=xin) x0=x[i],y0=y[i],k=i;
    double h=x[2]-x[1];
    double u=(xin-x0)/h;
    yout=y0;
    yout += (u*m[k][1]) + ((u*(u-1.0)/2.0)*m[k][2]) + ((u*(u-1.0)*(u-2.0)/6.0)*m[k][3]);;
    cout<<"\n\nf("<<xin<<") = "<<yout<<endl;
}
